...
import com.asifiqbal.todotracking.features.todo.taskreminder.ui.TaskBroadcastReceiver
import com.asifiqbal.todotracking.foundation.extension.ellipsisAt
import com.asifiqbal.todotracking.foundation.localization.LocalizationUtil
import com.asifiqbal.todotracking.foundation.theme.LightError
...
class TaskNotificationManager @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    private fun initChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val description =
                getLocalizedContext().getString(R.string.todo_task_channel_description)
            val importance = NotificationManager.IMPORTANCE_HIGH

            NotificationChannel(CHANNEL_ID, name, importance).apply {
                this.description = description
                enableLights(true)
                lightColor = ResourcesCompat.getColor(
                    getLocalizedContext().resources,
        ...
        return NotificationCompat.Builder(context, CHANNEL_ID).apply {
            ...
            setStyle(
                NotificationCompat.BigTextStyle().bigText(
                    task.name.ellipsisAt(100) + "\n" + task.itemInfoDisplayable(
                        getLocalizedContext().resources,
                        LightError
                    )
                )
    ...
    private fun getCompleteAction(taskId: String): NotificationCompat.Action {
        ...
        val intent = getIntent(
            taskId,
            TaskBroadcastReceiver.ACTION_NOTIFICATION_COMPLETED,
            REQUEST_CODE_ACTION_COMPLETE
        )
    ...
    private fun getSnoozeAction(taskId: String): NotificationCompat.Action {
        val actionTitle =
            getLocalizedContext().getString(R.string.todo_task_notification_action_snooze)
        val intent = getIntent(
            taskId,
            TaskBroadcastReceiver.ACTION_NOTIFICATION_SNOOZE,
            REQUEST_CODE_ACTION_SNOOZE
        )
        return NotificationCompat.Action(ACTION_NO_ICON, actionTitle, intent)
    }
