...
import com.asifiqbal.todotracking.foundation.extension.ellipsisAt
...
fun ToDoTask.itemInfoDisplayable(
    resources: Resources,
    expiredColor: Color,
    listName: String = ""
): AnnotatedString? {
...
fun ToDoTask.dueDateDisplayable(
    resources: Resources,
    currentDate: LocalDateTime = DateTimeProviderImpl().now()
): String? {
...
fun LocalDate.headerDateDisplayable(
    resources: Resources,
    currentDate: LocalDateTime = DateTimeProviderImpl().now()
): String {
    ...
    return when (status) {
        ToDoStatus.IN_PROGRESS -> {
            when {
                createdAt.isSameDay(currentDate) -> {
                    when {
                        ...
                        createdAt.isSameHour(currentDate) -> {
                            ...
                            stringResource(
                                R.string.todo_task_in_progress_date_creation_minute,
                                minutes.toString()
                            )
                        }
                            ...
                            stringResource(
                                R.string.todo_task_in_progress_date_creation_hour,
                                hours.toString()
                            )
                        }
                    }
                ...
                createdAt.isYesterday(currentDate) -> stringResource(
                    R.string.todo_task_in_progress_date_creation_date,
        ...
        ToDoStatus.COMPLETE -> {
            when {
                ...
                completedAt.isYesterday(currentDate) -> stringResource(R.string.todo_task_complete_date_yesterday)
                    ...
                    R.string.todo_task_complete_date,
...
fun ToDoTask.noteUpdatedAtDisplayable(currentDate: LocalDateTime = DateTimeProviderImpl().now()): String {
    return when {
        ...
        noteUpdatedAt.isYesterday(currentDate) -> stringResource(
            R.string.todo_task_note_update_yesterday,
            createdAt.formatDateTime()
        )
...
private fun ToDoTask.totalStepInfo(resources: Resources): String {
    ...
    return if (totalStep != 0) {
        ...
        resources.getString(
            R.string.todo_task_item_info,
            totalStepDone.toString(),
            totalStep.toString()
        )
    } else {
        ""
